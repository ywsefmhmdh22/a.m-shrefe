...
const WHATSAPP_NUMBER = "+201125571077"; 
// ✅ ملاحظة: تم تعديل الرقم ليحتوي على رمز الدولة (+20) لضمان عمل رابط الواتساب.
// ==========================================================
...
export default function ContactUsPage() {
    ...
    // ==========================================================
    // ✅ النص الطويل المضاف (لتلبية متطلبات جوجل أدسنس)
    // ==========================================================
    const longContactText = (
        <div className="text-right text-base text-gray-700 dark:text-gray-300 mb-8 mt-6 leading-relaxed" dir="rtl">
            <h2 className="text-xl font-bold text-blue-600 dark:text-blue-400 mb-3">
                نحن هنا من أجلك
            </h2>
            <p className="mb-4">
                تُعد صفحة **التواصل معنا** هي الجسر الذي يربطنا بعملائنا الكرام. نحن نؤمن بأن الخدمة الممتازة تبدأ من الاستماع الجيد لاحتياجاتك واستفساراتك. فريق دعم العملاء لدينا على أتم الاستعداد لتقديم المساعدة في أي وقت.
            </p>
            <p className="mb-4">
                سواء كان لديك **استفسار** حول أحد المنتجات الجديدة، أو تحتاج إلى **دعم فني** بشأن طلب سابق، أو لديك **اقتراح** لتطوير خدماتنا، أو حتى ترغب في تقديم **شكوى**، فإننا نرحب بكافة رسائلك. هدفنا هو الرد على جميع الاستفسارات في غضون 24 ساعة، مع إعطاء الأولوية للطلبات العاجلة. 
            </p>
            <p className="font-semibold">
                يرجى استخدام النموذج ادناه أو إحدى قنوات التواصل الاجتماعي الموضحة أدناه. تأكد من إدخال بريد إلكتروني صحيح لضمان وصول ردنا إليك.
            </p>
        </div>
    );
    // ==========================================================
    ...
    return (
        <main 
            className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-900 dark:to-gray-800 text-gray-800 dark:text-gray-100 flex items-center justify-center px-6 py-20"
            dir="rtl" // تطبيق الاتجاه العربي على الصفحة بالكامل
        >
            {/* ========================================================== */}
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label className="block mb-2 text-sm font-semibold text-right">البريد الإلكتروني</label>
                        <input
                            type="email"
                            required
                            value={email}
                            onChange={(e) => setEmail(e.target.value)}
                            className="w-full p-3 rounded-xl border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-600 text-right"
                            placeholder="ahmedsherife888@gmail.com"
                        />
                    </div>

                    <div>
                        <label className="block mb-2 text-sm font-semibold text-right">الرسالة</label>
                        <textarea
                            required
                            rows={5}
                            value={message}
